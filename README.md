...
Multi-user drawing app built with **Node.js + native WebSockets (`ws`) + Canvas API** (no drawing libraries).
...
- Keep the **same Room** value (default: `lobby`) to collaborate.
...
- **Undo/redo redraw cost**: undo/redo triggers a full canvas redraw (correct + simple; can be optimized with caching/tiling).
- **No persistence**: room state is in-memory; refreshes work (snapshot), but server restart resets.
- **Eraser model**: eraser is implemented via `destination-out` compositing; it’s recorded as a stroke operation.
- **Bandwidth**: points are throttled to animation frames; still can be heavy with many users.

## Time spent

- Replace this line with your actual time spent (e.g., “~6 hours”).
